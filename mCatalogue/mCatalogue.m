...
typedef struct{
  mCatalogueEntry *first;
  mCatalogueEntry *second;
} mCatalogueEntryPair;
...
@interface mCatalogueViewController(){
  ...
  BOOL itemViewsShoulShowPlaceholder;
}
...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
  ...
  if ( self )
    ...
    itemViewsShoulShowPlaceholder = NO;
...
//  [alert show];
  ...
  if(note){
    NSMutableArray *fields = [[userProfile.fields mutableCopy] autorelease];
...
{
  NSMutableDictionary *elementMap = [[[NSMutableDictionary alloc] init] autorelease];
                    ...
                    andProducts:(NSArray *)queriedProducts
{
  ...
  if (topAndHavetSubCategory && ([populatedCategories count] == 1) && ![queriedProducts count]){
    ...
    for(mCatalogueItem *item in category.items){
      ...
      itemViewsShoulShowPlaceholder |= item.hasImage;
    }
  } else {
        ...
        cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if(_catalogueParams.isGrid){
    ...
    if(!cell){
      cell = [[[mCatalogueGridCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier] autorelease];
    ...
    if(indexPath.row < categoriesCount){
      ...
      firstView = [[[mCatalogueCategoryView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleGrid] autorelease];
      ...
      if((id)secondCategory != (id)[NSNull null]){
        secondView = [[[mCatalogueCategoryView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleGrid] autorelease];
      ...
      mCatalogueItemView *firstItemView = [[[mCatalogueItemView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleGrid] autorelease];
      ...
      [firstItemView showPlaceholder:itemViewsShoulShowPlaceholder];
      ...
      if(catalogueEntryPair.count == 2){
        ...
        if(secondItem != (id)[NSNull null]){
          mCatalogueItemView *secondItemView = [[[mCatalogueItemView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleGrid] autorelease];
          secondView = secondItemView;
          
          secondItemView.catalogueItem = secondItem;
          [secondItemView showPlaceholder:itemViewsShoulShowPlaceholder];
    ...
    if(!cell){
      cell = [[[mCatalogueRowCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellIdentifier] autorelease];
      cell.userInteractionEnabled = YES;
      cell.selectionStyle = UITableViewCellSelectionStyleNone;
      cell.backgroundColor = self.catalogueTableView.backgroundColor;
    }
    ...
    if(indexPath.row < categoriesCount){
      ...
      catalogueEntryView = [[[mCatalogueCategoryView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleRow] autorelease];
     ...
     //it's rows, show screen-wide item
      ...
      mCatalogueItemView *itemView = [[[mCatalogueItemView alloc] initWithCatalogueEntryViewStyle:mCatalogueEntryViewStyleRow] autorelease];
      catalogueEntryView = itemView;
      
      mCatalogueItem *item = [items objectAtIndex:itemIndex];
      itemView.catalogueItem = item;
      [itemView showPlaceholder:itemViewsShoulShowPlaceholder];
...
-(CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath
{
  ...
  if(indexPath.row < categoriesCount){
    height = [mCatalogueCategoryView sizeForStyle:style withPlaceholder:NO].height;
    ...
    height = [mCatalogueItemView sizeForStyle:style
                              withPlaceholder:itemViewsShoulShowPlaceholder].height + rowGap;
  }
  return height;
}
...
-(void)showCategoryContent:(UITapGestureRecognizer *)recognizer
{
  ...
  categoryVC.showTabBar = NO;
  categoryVC.tabBarIsHidden = YES;
...
-(void)mCatalogueSearchViewDidShowSearchField{
  ...
  [cancelSearchGestureRecognizer release];
}
